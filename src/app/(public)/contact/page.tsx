...
import { getBusinessSettings, getProductBySlug, getVariantById } from "@/lib/data";
...
export default async function ContactPage({ searchParams }: Props) {
  ...
  const [settings, product, variant] = await Promise.all([
    getBusinessSettings(),
      ...
      <Container>
        ...
        <div className="mt-12 grid gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="rounded-md border border-border bg-white p-6 sm:p-8">
            ...
            <EnquiryForm productId={product?.id} variantId={variant?.id} contextLabel={contextLabel} />
          </div>
        </div>
      </Container>
    </Section>
